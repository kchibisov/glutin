...
impl Display {
    pub(crate) fn find_configs(
        ...
        let mut extra = ConfigExtraSupport::empty();
        if self.inner.client_extensions.contains("EGL_EXT_pixel_format_float") {
            extra |= ConfigExtraSupport::FLOAT_PIXELS;
        }
        ...
        if self.inner.client_extensions.contains("EGL_KHR_gl_colorspace") {
            extra |= ConfigExtraSupport::SRGB;
        }

        let configs = found_configs
            .into_iter()
            .map(move |raw| {
                let inner = Arc::new(ConfigInner { display: self.clone(), raw, extra });
                Config { inner }
            })
            .filter(move |_config| {
                #[cfg(x11_platform)]
                if template.transparency {
                    if let RawDisplayHandle::Xlib(display_handle) = self.inner.raw_display {
...
impl GlConfig for Config {
    ...
    fn float_pixels(&self) -> bool {
        if self.inner.extra.contains(ConfigExtraSupport::FLOAT_PIXELS) {
            matches!(
    ...
    fn srgb_capable(&self) -> bool {
        self.inner.extra.contains(ConfigExtraSupport::SRGB)
    }
...
impl X11GlConfigExt for Config {
    fn x11_visual(&self) -> Option<X11VisualInfo> {
        match self.inner.display.inner.raw_display {
            RawDisplayHandle::Xlib(display_handle) => unsafe {
