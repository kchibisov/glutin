...
pub type XlibErrorHookRegistrator =
...
/// The last error arrived from glx normallized by `GLX_BASE_ERROR`.
static LAST_GLX_ERROR: Lazy<Mutex<Option<Error>>> = Lazy::new(|| Mutex::new(None));

/// Store the last error comming from GLX.
...
fn last_glx_error(display: GlxDisplay) -> Result<()> {
    unsafe {
        // Force synchronization.
        (XLIB.as_ref().unwrap().XSync)(display.0 as *mut _, 0);
    }
