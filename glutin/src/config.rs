...
bitflags! {
    /// Extra config bits that require separate extensions.
    pub(crate) struct ConfigExtraSupport: u8 {
        const FLOAT_PIXELS = 0b0000_0001;
        const SRGB         = 0b0000_0010;
    }
}
...
pub struct ConfigTemplate {
    /// The type of the backing buffer and anccilary buffers.
    ...
    /// The rendering Apis supported by the configuration.
    pub(crate) api: Api,

    /// The config should support transparency.
    pub(crate) transparency: bool,

    /// The config supports double buffering.
    pub(crate) single_buffering: bool,

    /// The config suppots stereoscopy.
...
impl ConfigTemplateBuilder {
    ...
    /// # Api specific
    ///
    /// EGL on X11 doesn't provide a way to create a transparent surface. Use GLX for that instead.
    #[inline]
...
impl GlConfig for Config {
    fn color_buffer_type(&self) -> ColorBufferType {
        dispatch_gl!(self; Self(config) => config.color_buffer_type())
    }

    fn float_pixels(&self) -> bool {
        dispatch_gl!(self; Self(config) => config.float_pixels())
    }

    fn native_visual(&self) -> u32 {
        dispatch_gl!(self; Self(config) => config.native_visual())
    }

    fn alpha_size(&self) -> u8 {
        dispatch_gl!(self; Self(config) => config.alpha_size())
    }

    fn depth_size(&self) -> u8 {
        dispatch_gl!(self; Self(config) => config.depth_size())
    }

    fn stencil_size(&self) -> u8 {
        dispatch_gl!(self; Self(config) => config.stencil_size())
    }

    fn sample_buffers(&self) -> u8 {
        dispatch_gl!(self; Self(config) => config.sample_buffers())
    }

    fn srgb_capable(&self) -> bool {
        dispatch_gl!(self; Self(config) => config.srgb_capable())
    }

    fn config_surface_types(&self) -> ConfigSurfaceTypes {
        dispatch_gl!(self; Self(config) => config.config_surface_types())
    }

    fn api(&self) -> Api {
        dispatch_gl!(self; Self(config) => config.api())
    }
}
...
impl X11GlConfigExt for Config {
    fn x11_visual(&self) -> Option<X11VisualInfo> {
        dispatch_gl!(self; Self(config) => config.x11_visual())
    }
}
...
impl GetGlDisplay for Config {
    ...
    fn display(&self) -> Self::Target {
        dispatch_gl!(self; Self(config) => config.display(); as Display)
    }
}
...
impl AsRawConfig for Config {
    fn raw_config(&self) -> RawConfig {
        dispatch_gl!(self; Self(config) => config.raw_config())
    }
}
